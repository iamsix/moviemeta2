...
class fetcher(object):
    ...
    def __init__ (self, identifier):
        ...
        if identifier.find("tt") != -1:
            url = "http://api.themoviedb.org/2.1/Movie.imdbLookup/en/json/%s/%s" % (apikey, urllib2.quote(identifier))
        else:   
            url = "http://api.themoviedb.org/2.1/Movie.getInfo/en/json/%s/%s" % (apikey, urllib2.quote(identifier))
        opener = urllib2.build_opener()
        opener.addheaders = [('User-Agent',"Opera/9.10 (YourMom 8.0)"),
                             ('Range', "bytes=0-40960")]
        pagetmp = opener.open(url).read()
    ...
    def Studios(self):
        studios = []
        for genre in self.tmdbjson['studios']:
            studios.append(genre['name'])
        return studios
